...
import { aiApi, configApi, organizationApi } from '../api/client';
import type { LLMConfig, Organization } from '../api/types';
...
export default function Settings() {
  ...
  const saveRename = async () => {
    if (!renaming || orgBusy) return;
    const name = renaming.name.trim();
    if (!name) return;
    setOrgBusy(true);
    setError('');
    try {
      await organizationApi.update(renaming.id, { name });
      setRenaming(null);
      await reloadOrganizations();
    } catch (e) {
      setError(e instanceof Error ? e.message : String(e));
  ...
  const removeOrg = async (org: Organization) => {
    if (!window.confirm(`删除组织「${org.name}」？其中的人物不会删除，只会变为「未归属」。`)) return;
    setOrgBusy(true);
    setError('');
    try {
        ...
        <section className="min-w-0 flex-1 space-y-4">
          ...
          {tab === 'other' ? (
            <div className="rounded-xl border border-border bg-card p-5 shadow-sm">
              <h2 className="mb-3 text-sm font-semibold text-foreground">组织管理</h2>
              <div className="mb-4 flex gap-2">
                <input
                  value={newOrgName}
                  onChange={(e) => setNewOrgName(e.target.value)}
                  onKeyDown={(e) => e.key === 'Enter' && createOrg()}
                  placeholder="新组织名称，如：某某科技"
                  className={controlClass}
                />
                <Button onClick={createOrg} disabled={orgBusy || !newOrgName.trim()} variant="outline" className="shrink-0">
                  新建组织
                </Button>
              </div>
              {organizations.length === 0 ? (
                <p className="py-2 text-center text-sm text-muted-foreground">还没有组织。人物可以不归属任何组织。</p>
              ) : (
                  ...
                  {organizations.map((org) => (
                    <li key={org.id} className="flex items-center justify-between gap-3 py-2">
                      ...
                      <div className="flex shrink-0 gap-3">
                        {renaming?.id === org.id ? (
                          <>
                            <Button variant="link" size="sm" onClick={() => void saveRename()} className="px-0">
                              保存
                            </Button>
                            <Button variant="link" size="sm" onClick={() => setRenaming(null)} className="px-0 text-muted-foreground">
                              取消
                            </Button>
                          </>
                        ) : (
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => setRenaming({ id: org.id, name: org.name })}
              ...
              <p className="mt-3 text-xs text-muted-foreground">
                组织只是分组标签。删除组织不会删除其中的人物，成员会变为「未归属」。
              </p>
            </div>
          ) : null}
        </section>
