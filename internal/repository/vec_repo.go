...
type VecRepo struct {
	db *sql.DB
	// ready records whether the index was successfully initialised. Without it a
	// caller cannot tell "the search found nothing" from "there was no index to
	// search", and an empty result would be reported as if the timeline had
...
// Available reports whether the vector index is usable in this process.
func (r *VecRepo) Available() bool { return r.ready }
